...
ColorPicker(
    ...
    function (hex, hsv, rgb, pickerCoordinate, sliderCoordinate) {
        ...
        setColorRGB(rgb, hex);
        changeColor(hex)
        document.getElementById('colorBackground').innerHTML = hex;
    }
);

function setColorRGB(rgb, hex) {
    ...
    document.getElementById('colorBackground').style.backgroundColor = bgColor;
    document.getElementById('arrow-left').style.borderRight = '10px solid ' + bgColor;
    document.getElementById('arrow-right').style.borderLeft = '10px solid ' + bgColor;
    document.getElementById('colorBackground').style.color = textColor;
...
function drawGrid(lineCount, highlightNum) {
    if (lc !== lineCount) {
        resetTiles(lineCount)
    }
    document.getElementById('horizontalVal').value = lineCount;
    document.getElementById('verticalVal').value = lineCount;
    ...
    for (var i = 0; i < lineCount; i++) {
        ...
        ctx.moveTo(0, y);
        ctx.lineTo(ch, y);
        const tileSize = (cw / lineCount);
        for (let y1 = 0; y1 < canvas1.width / tileSize; y1++) {
            for (let x1 = 0; x1 < canvas1.height / tileSize; x1++) {
                const tileNum = x1 + canvas1.width / tileSize * y1;
                const xx = x1 * tileSize;
                const yy = y1 * tileSize;
...
document.getElementById('horizontalVal').onkeyup = function (event) {
    if (event.target.value > 40) {
        alert('Limit between 1 to 40');
        document.getElementById('horizontalVal').value = lc;
        document.getElementById('verticalVal').value = lc;
    } else {
        drawGrid(event.target.value);
    }
}

document.getElementById('verticalVal').onkeyup = function (event) {
    if (event.target.value > 40) {
        alert('Limit between 1 to 40');
        document.getElementById('horizontalVal').value = lc;
        document.getElementById('verticalVal').value = lc;
...
function handleClick(evt) {
    ...
    let tileNum = tileX + canvas1.width / tileSize * tileY;
    if (tileNum !== lastTile) {
        lastTile = tileNum;
...
function changeColor(color = '#00a0ff') {
    boxColor = color;
}
changeColor();
drawGrid(lc);
...
document.getElementById('downloadImage').onclick = function () {
    document.getElementById("downloadImage").style.border = "2px solid";
